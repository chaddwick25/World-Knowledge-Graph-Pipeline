#!/usr/bin/env python
"""Generate data/subgraph_coverage_metrics.csv from the Django DB.

This is a thin wrapper around the `export_subgraph_coverage_metrics` Django
management command, to match the existing scripts/ ergonomics.

Usage (from project root):

    poetry run python scripts/export_subgraph_coverage_metrics.py

You can pass additional arguments through to the management command, e.g.:

    poetry run python scripts/export_subgraph_coverage_metrics.py \
        --output data/custom_subgraph_coverage_metrics.csv
"""
...
def main(argv: List[str]) -> None:
    ...
    # `export_subgraph_coverage_metrics` management command so you can
    # override e.g. --output if desired.
    call_command("export_subgraph_coverage_metrics", *argv)
