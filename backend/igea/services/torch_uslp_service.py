...
from igea.services.spatial_link_prediction import RELATION_GEOHASH_PRECISION, RELATION_TO_NATURAL_TEXT
...
class TorchUSLP(GPUAcceleratedUSLP):
    ...
    def score_batch(
        ...
        Returns (H, N) normalized score matrix.
        """
        ...
        del a
        geo_scores = 1.0 / (1.0 + dist_km_centers)
        del dist_km_centers
    ...
    def predict_links_batched(
        ...
        for relation, heads_with_literals in relation_groups.items():
            for chunk_start in range(0, len(heads_with_literals), batch_size):
                ...
                scores = self.score_batch(lit_embs, lats, lons, relation)
                ...
                for i, head in enumerate(chunk):
                    ...
                    for score_val, pool_idx in zip(top_vals.cpu().tolist(), masked_pool_idxs.cpu().tolist()):
                        all_links.append({
