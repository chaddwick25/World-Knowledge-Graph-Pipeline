...
Updated 2026-09-09 to the current ``_score_candidates_gpu`` API
(``head_lat``/``head_lon``/``precision`` args and the paper's
``1 - d/d_max`` geo formula on geohash cluster centers — Mann et al.
§3.3, Fix 1).

These are GPU integration tests (designed for the RTX 4070 Ti SUPER,
16 GB). They skip cleanly in environments without CUDA (e.g. the
backend container); run them in the worker container or a GPU host.
"""
import pytest
import torch
import numpy as np
...
def _seed_pool_data(service, coords, d_max):
    """Populate the per-precision geo-scoring tensors used by
    ``_score_candidates_gpu`` (cluster centers + per-row d_max)."""
    n = coords.shape[0]
    service._gpu_pool_coords = coords
    service._gpu_pool_center_coords = {4: coords}
    service._gpu_pool_d_max = {4: torch.full((n,), d_max, device=service.device)}
    service._gpu_pool_embeddings = torch.randn(n, 300, device=service.device)
    service._gpu_pool_class_embeddings = torch.randn(n, 300, device=service.device)


class TestGPUUSLPScoring:
    """Test GPU USLP scoring logic."""

    def test_score_candidates_gpu_returns_correct_shape(self, mock_gpu_fasttext, sample_candidate_pool):
        ...
        _seed_pool_data(service, coords, d_max=100.0)
        ...
        total_scores, geo_scores, name_scores, class_scores = service._score_candidates_gpu(
            literal, relation, distances, indices, 17.45, -88.45, 4
        )
    ...
    def test_geo_score_decreases_with_distance(self, mock_gpu_fasttext, sample_candidate_pool):
        """Test that geo score decreases as distance increases.

        Geo score is ``1 - d/d_max`` (Mann et al. §3.3) on geohash cluster
        centers. Candidates are spaced ~55 km apart so a P4 cluster-center
        offset of up to ~20 km can't reorder the distances.
        """
        service = GPUAcceleratedUSLP(device='cuda:0')
        service._pool = sample_candidate_pool
        service.gpu_fasttext = mock_gpu_fasttext
        service.gpu_haversine = Mock()
        # Candidates at (0,0), (1,0), (2,0): ~111 km apart in lon.
        coords = torch.tensor([[0.0, 0.0], [1.0, 0.0], [2.0, 0.0]], device=service.device)
        _seed_pool_data(service, coords, d_max=300.0)

        # Distances arg is only used for upstream radius filtering — the geo
        # score derives from cluster-center haversine, so we pass dummies.
        distances = torch.tensor([0.1, 1.0, 5.0], device=service.device)
        indices = torch.tensor([0, 1, 2], device=service.device)

        total_scores, geo_scores, _, _ = service._score_candidates_gpu(
            "test", "test", distances, indices, 0.0, 0.5, 4
        )

        # Geo formula: 1 - d/d_max — decreasing in center-to-center distance
        assert geo_scores[0] > geo_scores[1] > geo_scores[2]

    def test_normalization_divides_by_three(self, mock_gpu_fasttext, sample_candidate_pool):
        ...
        total_scores, geo_scores, name_scores, class_scores = service._score_candidates_gpu(
            "test", "test", distances, indices, 17.45, -88.45, 4
        )
    ...
    def test_threshold_filtering_works_correctly(self, mock_gpu_fasttext, sample_candidate_pool):
...
class TestGPUUSLPPredictionFlow:
    ...
    def test_predict_links_for_entity_with_zero_candidates(self, mock_gpu_fasttext, mock_gpu_haversine):
    ...
    def test_predict_links_for_entity_filters_self(self, mock_gpu_fasttext, mock_gpu_haversine):
