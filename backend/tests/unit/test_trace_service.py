...
def _clean_trace(monkeypatch):
    """Every test starts with clean TRACE_* env and fresh singletons."""
    for var in (
...
class TestTraceService:
    ...
    def test_drop_oldest_on_overflow(self, monkeypatch, tmp_path):
        monkeypatch.setenv("TRACE_SINK", "file")
        monkeypatch.setenv("TRACE_SINK_URL", str(tmp_path / "trace.ndjson"))
        monkeypatch.setenv("TRACE_QUEUE_SIZE", "4")
    ...
    def test_sink_down_fail_soft(self, monkeypatch):
        monkeypatch.setenv("TRACE_SINK", "langfuse")
        monkeypatch.setenv("TRACE_SINK_URL", "http://127.0.0.1:1/api/public")
        monkeypatch.setenv("TRACE_SINK_PUBLIC_KEY", "pk")
        monkeypatch.setenv("TRACE_SINK_SECRET_KEY", "sk")
        monkeypatch.setattr(TraceService, "_ensure_thread", lambda cls: None)
