...
class GeoVectorsEncoderService:
    ...
    def generate_pickle(self, country_name, continent=None):
        """
        Procedurally generates a wdw.pickle for a country using local TSV files.
        ...
        if not relations:
            logger.error("GeoVectors: No country relations available from resolver")
            return {"success": False, "error": "Country relations not available"}
        ...
        if not mapping:
            logger.info(
                "GeoVectors: Skipping pickle for %s - No country mapping found in resolver.",
                country_name,
            )
            return {"success": True, "skipped": True}

        tsv_rel = mapping.get("geovectors_location_tsv")
        if not tsv_rel:
            logger.info(
                "GeoVectors: Skipping pickle for %s - No local embeddings found in metadata.",
                country_name,
            )
            return {"success": True, "skipped": True}

        tsv_path = Path(tsv_rel)
        # Resolve relative paths against EMBEDDINGS_ROOT
        if not tsv_path.is_absolute():
            tsv_path = Path(settings.EMBEDDINGS_ROOT) / tsv_rel

        if not tsv_path.exists():
            logger.warning(f"GeoVectors: TSV file not found at {tsv_path}")
            return {'success': False, 'error': 'TSV file not found'}
        
        # 2. Identify regional directory
        if not continent:
        ...
        # Root regional outputs under the OSM_WIKIDATA_EXTRACTIONS_DIR canonical root
        base_dir = Path(settings.OSM_WIKIDATA_EXTRACTIONS_DIR) / continent.lower() / normalize_country_slug(country_name)
        pickle_dir = base_dir / "pickle"
        pickle_dir.mkdir(parents=True, exist_ok=True)
        output_pickle = pickle_dir / "wdw.pickle"

        # 3. Handle Geo-Fencing (Polyfile)
        # We look for the polyfile in the temporal_snapshots directory (Phase 3 output)
        poly_path = None
        snap_dir = base_dir / "temporal_snapshots"
        polys = list(snap_dir.glob("*.poly"))
        if polys:
            poly_path = polys[0] # Use the first available snapshot boundary

        boundary = self._parse_poly_file(poly_path) if poly_path else None
        if boundary:
            logger.info(f"GeoVectors: Using boundary for {country_name} masking.")

        # 4. Process TSV -> Pickle
        logger.info(f"GeoVectors: Building spatial index for {country_name} from {tsv_path}...")
        ...
        with gzip.open(tsv_path, 'rt') as f:
            for line in f:
                ...
                # key format for NLEModel is {type}_{id}
                # Map short 'n', 'w', 'r' to 'node', 'way', 'relation'
        ...
        # 4a. Persist legacy pickle format for backward compatibility
        with open(output_pickle, 'wb') as f:
            pickle.dump(wdw_store, f)

        # 4b. Also persist NumPy + JSON index for optimized NLE loading (3.2)
        try:
            import json

            embeddings = np.zeros((len(wdw_store), wdw_store.n_components), dtype=np.float32)
            keys = []
            for i, (key, vec) in enumerate(wdw_store.items()):
                keys.append(key)
                embeddings[i] = vec

            np.save(pickle_dir / "wdw_embeddings.npy", embeddings)
            with open(pickle_dir / "wdw_keys.json", "w", encoding="utf-8") as jf:
                json.dump(keys, jf)
        except Exception as exc:
            logger.warning(
                "GeoVectors: Failed to write NumPy/JSON NLE index for %s: %s",
                country_name,
                exc,
            )

        logger.info(f"GeoVectors: SUCCESS - Generated wdw.pickle with {len(wdw_store)} entities.")
        return {'success': True, 'path': str(output_pickle), 'entities': len(wdw_store)}

    @staticmethod
    def _parse_poly_file(poly_path):
        """Parses an OSM .poly file into a shapely Polygon."""
        coords = []
        try:
            with open(poly_path, 'r') as f:
                ...
                for line in lines:
                    line = line.strip()
                    if not line or line == "END" or line.isalpha(): continue
                    parts = line.split()
                    if len(parts) == 2:
