"""Country bounding-box resolution.
...
from core.services.planet_init.geometry_utils import (
    parse_poly_bbox,
    expand_bbox_to_min_span,
    MIN_BBOX_SPAN,
)
from core.services.planet_init.country_iso import resolve_iso_code
from core.services.planet_init.country_relations import get_country_by_iso
    ...
    try:
        ...
        with connections["vectors"].cursor() as cursor:
            ...
            if row and row[0]:
                ...
                # ST_Extent on an empty result set returns "BOX(NaN NaN,NaN NaN)"
                # instead of NULL. Reject it early — NaN coordinates crash GEOS.
                if "NaN" in extent_str:

                    logger.debug(

                        "ST_Extent returned NaN for country '%s' (no matching rows); skipping",
...
def populate_bbox_for_profile(profile) -> Optional[Tuple[float, float, float, float]]:
    ...
    if country_pbf is not None:
        ...
        try:
            ...
            from core.models import OsmiumDatasetMetrics
...
def resolve_bbox_for_profile(profile) -> Optional[Tuple[float, float, float, float]]:
    ...
    core.models.CountryPipelineProfile, but is kept loosely
