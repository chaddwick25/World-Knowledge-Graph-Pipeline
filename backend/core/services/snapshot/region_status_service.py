...
from core.models import PbfFile, RegionHierarchy, PolygonFile
from core.models import ProcessingSession, Task
...
class RegionStatusService:
    ...
    def _check_semantic_search_status(pbf: PbfFile) -> Dict:
        ...
        try:
            ...
            # Resolve the Snapshot UUID for this PbfFile.
            # OsmEntity.source_snapshot_id stores the Snapshot UUID, not the
            # PbfFile UUID.  Older code incorrectly used pbf.id here.
            snapshot = Snapshot.objects.filter(pbf_file=pbf).first()
            if snapshot is None:
                return {'available': False}
            snapshot_uuid = str(snapshot.id)

            # Check if any entities exist for this snapshot
    ...
    def _check_asset_bundle_status(pbf: PbfFile) -> Dict:
        """Check if graph extracts exist for this PBF's snapshot.

        AssetBundle (legacy) has been removed.  GraphExtract (osmsnapshot)
        replaces it, but is keyed by Snapshot, not PbfFile directly.
        """
        try:
            from osmsnapshot.models import GraphExtract

            extracts = GraphExtract.objects.filter(
                snapshot__pbf_file=pbf
            )
            extract_count = extracts.count()
            if extract_count > 0:
                return {
                    'available': True,
                    'bundle_count': extract_count,
                    'tag_keys': [],
                }
            return {'available': False}
        except Exception as e:
            logger.warning(f"Could not check graph extract status: {str(e)}")
            return {'available': False}
