...
class AssetExtractor(osmium.SimpleHandler):
    ...
    def way(self, w):
        try:
            # NOTE: Way centroids are NOT written to nodes.parquet.
            # Previous versions appended way_id + centroid as pseudo-nodes,
            # which polluted the nodes table with fake entries whose osm_id
            # was a way ID, not a node ID.  If way centroid coordinates are
            # needed for GNN features, compute them downstream from
            # edges.parquet + nodes.parquet instead.
