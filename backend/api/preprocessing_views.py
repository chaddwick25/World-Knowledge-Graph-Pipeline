...
class CountrySearchUpdateView(APIView):
    ...
    def _execute_pipeline(self, country_name, region_id, session, country_processing):
        ...
        try:
            ...
            # Update graph-asset metadata; final is_processed will be set after GeoVectors encoding
...
class CountrySearchStatusView(APIView):
    ...
    def get(self, request, country_name):
        ...
        try:
            # 1. Identify continent for the country
            region = RegionHierarchy.objects.filter(name__iexact=country_name).first()
            continent = region.parent.name if region and region.parent else None
            
            # 2. Check Filesystem for required artifacts (2021-2025)
            # This implements the user's requirement for strict history/snapshots/polys tracking
            history_count = 0
            snapshot_count = 0
            poly_count = 0
            ...
            if continent:
                ...
                for year in range(2021, 2026):
                    history_path = regional_path_service.get_yearly_pbf_path(continent, country_name, year)
                ...
                for year in range(2021, 2026):
                    for month in range(1, 13):
                        ...
                        snap_path = regional_path_service.get_snapshot_pbf_path(continent, country_name, year, month, last_day)
                        if snap_path.exists():
                            snapshot_count += 1
                            if snap_path.with_suffix('.poly').exists():
                                poly_count += 1

            # 3. Check for wdw.pickle (GeoVectors spatial index)
            # Check both country-level pickle and subgraph pickles
            has_pickle = False
            if continent:
                # Check country-level pickle
                pickle_path = regional_path_service.get_pickle_path(continent, country_name)
                if pickle_path.exists():
                    has_pickle = True
                else:
                    # Check for subgraph pickles in pickles/ directory
                    country_dir = regional_path_service.get_country_dir(continent, country_name)
                    pickles_dir = country_dir / 'pickles'
            ...
            # 4. Get DB processing state if available
            try:
                country_processing = CountrySearchProcessing.objects.get(country_name=country_name)
                is_db_processed = country_processing.is_processed
                started_at = country_processing.processing_started_at
                completed_at = country_processing.processing_completed_at
            except CountrySearchProcessing.DoesNotExist:
                is_db_processed = False
                started_at = None
                completed_at = None

            # 5. Determine final 'Ready' state
            # A country is processed if:
            # - DB marks it as processed (is_db_processed) - this is the authoritative source after successful preprocessing
            # OR
            # - It has at least some snapshots AND matching polys (legacy filesystem-based check)
            is_processed = is_db_processed

            return Response({
                'country_name': country_name,
                'is_processed': is_processed,
                'is_db_processed': is_db_processed,
                'history_count': history_count,
                'monthly_extracts_count': snapshot_count,
