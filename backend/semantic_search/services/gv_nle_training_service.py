"""GV-NLE training pipeline service (k-NN graph → DeepWalk → DB write-back).

Extracted from the ``train_gv_nle`` management command (monolith split,
Phase 5 of PIPELINE_CONTROL_PLANE_AND_CLEANUP_PLAN).  The command is now a
thin wrapper; subgraph workers (Pool children) call this service directly.
"""
...
def _train_subgraph_worker(task_data):
    ...
    try:
        service = GVNLETrainingService(stdout=sys.stdout)
        service.run(**subgraph_options)
...
class GVNLETrainingService:
    """GV-NLE training pipeline: k-NN graph, weighted DeepWalk, DB write-back.

    Drop-in replacement for the ``train_gv_nle`` command body.  ``run()``
    mirrors the command's ``handle()`` dispatch (subgraph mode vs country /
    region mode); ``stdout``/``style`` are duck-typed (``OutputWrapper`` or
    ``StringIO``).
    """
    ...
    def run(self, **options):
        """Entry point — dispatches to subgraph mode or single-region train."""
        isos = options.get('isos')
        if isos:
            return self.process_subgraphs(isos, options)
    ...
    def train(self, **options):
        ...
        if buffer_wkt:
            ...
            # Scope to country_code / snapshot_id to avoid scanning all
            # partitions after cutover.  Falls back to full scan on the
        ...
        with transaction.atomic(using='vectors'):
            with connections['vectors'].cursor() as cursor:
                ...
                # Scope the UPDATE by partition keys (snapshot_id,
    ...
    def process_subgraphs(self, iso: str, options: dict):
        """
