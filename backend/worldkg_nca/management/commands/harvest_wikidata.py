...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Generate FastText embeddings for Wikidata candidates using their labels. "
                "Required for IGEA similarity scoring. Without this, all scores will be 0."
            ),
        )
        parser.add_argument(
            "--dry-run",
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.6,
            help="IGEA cosine acceptance threshold tha (default: 0.6).",
        )
    ...
    def handle(self, *args, **options):
        ...
        if cache_file and os.path.exists(cache_file):
            ...
            # Optional phase-3: Generate FastText embeddings from labels + pseudo-tags
            if options.get("generate_embeddings") and candidates:
                self.stdout.write(
                    "[1c/2] Generating FastText embeddings from Wikidata labels + pseudo-tags..."
                )
                from semantic_search.services.model_registry_service import ModelRegistryService
                ft_model = ModelRegistryService.get_fasttext_model()
                ontology_service = WorldKGOntologyService()
                
                import numpy as np
                for c in candidates:
                    tokens = []
                    # 1. Include label tokens (as if they were in a 'name' tag)
                    label = c.get("label", "")
                    if label:
                        tokens.append("name")
                        tokens.extend(label.split())
                    
                    # 2. Include tokens from canonical tags of the assigned class
                    wkg_cls = c.get("wkg_class")
                    if wkg_cls:
                        tags = ontology_service.get_canonical_tags(wkg_cls)
                    ...
                    if tokens:
                        vectors = [ft_model.get_word_vector(t) for t in tokens if t]
                        if vectors:
                ...
                with_emb = sum(1 for c in candidates if c.get("embedding"))
                self.stdout.write(self.style.SUCCESS(
                    f"  ✓ Generated embeddings for {with_emb:,}/{len(candidates):,} candidates"
                ))
            elif not options.get("generate_embeddings"):
                self.stdout.write(
                    "  (embeddings=None for all; add --generate-embeddings for IGEA scoring)"
                )
        ...
        # Parse poly-file to WKT for IGEA geofencing
        polygon_wkt = None
        if poly_file:
            from worldkg_nca.services.wikidata_service import parse_poly_to_wkt
            polygon_wkt = parse_poly_to_wkt(poly_file)
            if polygon_wkt:
                self.stdout.write(f"IGEA: using polygon geofence from {poly_file}")
            else:
                self.stdout.write(self.style.WARNING(
                    f"Could not parse poly-file for IGEA geofencing: {poly_file}"
                ))
        ...
        igea.load_wikidata_candidates(candidates)
        stats = igea.run(country_code=country, polygon_wkt=polygon_wkt)
